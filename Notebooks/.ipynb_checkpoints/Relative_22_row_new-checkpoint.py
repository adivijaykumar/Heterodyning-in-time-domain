...
class RelativeBinningTimeDomain_H1detectorframe_for_longer_signals(bilby.Likelihood):
    ...
    def __init__(
        ...
        self.gamma = np.array([5 / 8, 3 / 8, 1 / 4, -3 / 8]) # [5 / 8, 3 / 8, 1 / 4, -3 / 8]
        self.chi = 1
        self.time = time
    ...
    def compute_SNR_TD_and_waveform_data(self):
        ...
        for k in self.Detectors_list.keys():
            t_delay_fiducial = self.Detectors_list[k].time_delay_from_earth_center(
                right_ascension=self.fiducial_parameters["ra"],
                declination=self.fiducial_parameters["dec"],
                t_gps=self.fiducial_parameters["geocent_time"],
            )
            Time_array_per_detector[k] = self.time
            h22_fiducial[k] = self.waveform_22_modes_fiducial(
                self.fiducial_parameters, Time_array_per_detector[k] - (t_delay_fiducial+ self.fiducial_parameters["geocent_time"] - self.trigger_time)
            )
            if self.injection_parameters is not None:
                t_delay_injection = self.Detectors_list[k].time_delay_from_earth_center(
                ...
                h_injection[k] = self.waveform(
                    ...
                    Time_array_per_detector[k] - (t_delay_injection + self.fiducial_parameters["geocent_time"] - self.trigger_time),
                )
    ...
    def waveform_22_modes_fiducial(
        ...
        # h22[Time_array>0.04] = h22[Time_array>0.04]*1e50
        return h22
    ...
    def setup_bins_per_detector(self):
        ...
        for k in self.Detectors_list.keys():
            time_on_edge, number_of_bins, bin_edge_index, bin_centre, bin_width = self.setup_bins(
                self.Time_array_per_detector["L1"],
    ...
    def log_likelihood_ratio(self):
        ...
        for k in self.Detectors_list.keys():
            ...
            h22[self.time_on_edge[k]>0.05] = 0.0 + 1j*0.0
...
# Set fiducial parameters for longer signals:

class Set_Fiducial_parameters_for_longer_signals(RelativeBinningTimeDomain_H1detectorframe_for_longer_signals):
    def __init__(
        self,
        time,
        Detectors_list,
        ...
        super().__init__(
            time, Detectors_list, fiducial_parameters, injection_parameters, Data_list, C_inv_row, Noise, trigger_time, epsilon_array_per_detector, spacing, spacing_times
        )
        self.fiducial_parameters = fiducial_parameters.copy()
